...
algorithm
  ...
  if (Modelica.Utilities.Strings.find(
      ...
      "file://",
      startIndex=1,
      caseSensitive=false) == 0 and Modelica.Utilities.Strings.find(
      uri,
      "modelica://",
      startIndex=1,
      caseSensitive=false) == 0) then
    // try file://+uri
...
Now the function works from any directory as long as the <code>dhcSim</code> library
