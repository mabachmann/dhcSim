...
partial model Carnot
  ...
  parameter Modelica.SIunits.HeatFlowRate QEvaSet=0.0 "Constant thermal evaporator load" annotation (Dialog(enable=not use_Q_flow_in));
  parameter dhcSim.Fluid.Types.OutletTemperature effInpCon=dhcSim.Fluid.Types.OutletTemperature.gradient
    ...
    annotation (Dialog(group="Efficiency"), Evaluate=not use_TCon_in);
  ...
  Modelica.Blocks.Sources.RealExpression m_in(y=m_flow_calc) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-80,40})));
  ...
  Modelica.Blocks.Interfaces.RealInput QEva_in(unit="W") if  use_Q_flow_in
    "Heat load" annotation (Placement(transformation(
        extent={{-20,-20},{20,20}},
        rotation=90,
        origin={0,-120})));
...
equation
  ...
  connect(m_in.y,idealSource. m_flow_in) annotation (Line(points={{-69,40},{-56,
          40},{-56,52}},                                                                         color={0,0,127}));
